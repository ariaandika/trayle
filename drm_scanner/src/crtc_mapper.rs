...
impl SimpleCrtcMapper {
    ...
    fn restored_for_connector(&self, drm: &impl ControlDevice, connector: &connector::Info) -> Option<crtc::Handle> {
        let encoder = connector.current_encoder()?;
        let encoder = drm.get_encoder(encoder).ok()?;
        let crtc = encoder.crtc()?;connector.current_encoder();
        self.is_available(&crtc).then_some(crtc)
    }

    fn pick_next_available_for_connector(&self, drm: &impl ControlDevice, connector: &connector::Info) -> Option<crtc::Handle> {
...
impl CrtcMapper for SimpleCrtcMapper {
    fn map<'a>(
        ...
        for connector in connectors.clone().filter(|conn|conn.state() != connector::State::Connected) {
